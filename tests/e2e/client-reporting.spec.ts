...
test.describe("client reporting", () => {
  test("supports client comments and asset attachment on visible requests", async ({
    ...
    await expect(
      page.getByRole("heading", { name: "Weekly request summary for launch reviewers" })
    ).toBeVisible();
    ...
    await expect(page.getByText("DigiColony is actively working on this request.")).toBeVisible();
  ...
  test("lands client users on the reporting form and creates reports", async ({
    ...
    await expect(page.getByRole("link", { exact: true, name: "Report" })).toBeVisible();
