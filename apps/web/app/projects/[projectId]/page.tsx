...
export default async function ProjectWorkspacePage({
  ...
  const [projects, allItems, projectDetails] = await Promise.all([
  ...
  const activeCount = projectItems.filter((item) => item.pipelineStatusLabel !== "Done").length;
  const doneCount = projectItems.filter((item) => item.pipelineStatusLabel === "Done").length;
        ...
        <div className="grid gap-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3 text-sm">
              <Link className="font-bold text-indigo-600 hover:underline" href="/clients">
                Clients
              </Link>
              <span className="text-soft">/</span>
              <Link className="max-w-[28ch] truncate font-bold text-indigo-600 hover:underline" href={`/clients/${project.clientId}`}>
                {project.clientName ?? project.clientId}
              </Link>
            </div>
            <Link className={buttonClass("secondary")} href={`/work-items?projectId=${project.id}`}>
              Open main board
            </Link>
          </div>
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
            <Panel>
              <details className="inline-edit-control">
                <summary className="mb-4 flex cursor-pointer list-none items-start justify-between gap-3">
                  <span>
                    <span className="block text-lg font-bold tracking-tight text-ink">Project context</span>
                    <span className="mt-1 block text-sm leading-6 text-muted">Background, goals, and structured operating notes for this project.</span>
                  </span>
                  {admin ? (
                    ...
                    <>
                      <div className="grid gap-4 lg:grid-cols-2">
                        <div className="rounded-2xl bg-blue-soft p-4">
                          <h2 className="text-sm font-bold text-ink">Background</h2>
                          <p className="mt-2 text-sm leading-6 text-muted">
                            {projectDetails?.description ?? "No project background captured yet."}
                          </p>
                        </div>
                        <div className="rounded-2xl bg-blue-soft p-4">
                          <h2 className="text-sm font-bold text-ink">Goals</h2>
                          <p className="mt-2 text-sm leading-6 text-muted">
                            {projectDetails?.aiSummary ?? "No goals or operating summary captured yet."}
                          </p>
                        </div>
                      </div>
                      <div className="mt-4 rounded-2xl bg-ink p-4">
                        <h2 className="text-sm font-bold text-white">Structured context</h2>
                ...
                {admin ? (
                  <form action={updateProjectAction} className="inline-edit-form rounded-2xl border border-line bg-white p-4">
                    ...
                    <textarea className={`mt-3 ${fieldClass}`} defaultValue={projectDetails?.description ?? ""} name="description" placeholder="Background / context" rows={3} />
                    <textarea className={`mt-3 ${fieldClass}`} defaultValue={projectDetails?.aiSummary ?? ""} name="aiSummary" placeholder="Goals / operating summary" rows={4} />
                    <textarea className={`mt-3 ${fieldClass}`} defaultValue={projectDetails?.structuredContext ? JSON.stringify(projectDetails.structuredContext, null, 2) : ""} name="structuredContext" placeholder='Structured context JSON, for example {"goals":[],"background":""}' rows={6} />
            ...
            <Panel>
              ...
              <Link className={`mt-4 w-full ${buttonClass("primary")}`} href={`/work-items?projectId=${project.id}`}>
                Review on main board
              </Link>
              <div className="mt-4 grid gap-3">
                {projectItems.slice(0, 5).map((item) => (
                  <Link key={item.id} className="rounded-2xl border border-line p-3 hover:bg-blue-soft" href={`/work-items/${item.id}`}>
                    <p className="line-clamp-2 text-sm font-bold text-ink">{item.title}</p>
                    <p className="mt-2 text-xs text-muted">{item.pipelineStatusLabel} / {item.type}</p>
                  </Link>
                ))}
                {projectItems.length === 0 ? (
                  <p className="text-sm text-muted">No work items yet. Use the global Add menu to create one.</p>
                ) : null}
              </div>
            </Panel>
          </div>
