...
| `docs/runbooks/work-items-database-operations.md`  | External PostgreSQL metadata, readiness, backup/restore, and failure contract  | Before database inventory, migration, backup, restore, deployment, or rollback  |
| `docs/runbooks/work-items-garage-operations.md`    | Garage platform dependency, isolation, readiness, backup, and restore contract | Before storage inventory, policy, topology, backup, restore, or deployment      |
| `docs/runbooks/work-items-release-and-rollback.md` | Exact-source, rollout, drift, and application-only rollback contract           | Before schema-2 plan, deployment, cleanup, or rollback                          |
...
| `scripts/inventory-work-items-database-on-sheldon-remote.sh`                                | Print bounded read-only database metadata and protected counts                                                                                       | Run on Sheldon; never prints the database URL or password          |
| `scripts/inventory-work-items-garage-on-sheldon-remote.sh`                                  | Print Garage health, statistics, bucket/key policy metadata, and volume identities                                                                   | Run on Sheldon; does not request the secret key or object contents |
| `scripts/test-sheldon-readiness-containers.sh`                                              | Exercise candidate auth, permissions, file flow, dependency failure/recovery, isolation, and runtime limits against disposable PostgreSQL and Garage | Local Docker only; removes its isolated containers and volumes     |
| `scripts/test-server-action-release-skew.sh`                                                | Replace a disposable release A container with release B and submit a stale Server Action form using a test-only shared key                           | Local Docker only; never migrates or downgrades a database         |
| `pnpm --filter @digicolony/db storage:verify-isolation`                                     | Prove own-bucket access and explicit foreign-bucket denial                                                                                           | Requires a real distinct bucket and server-only `S3_*` values      |
