...
| `12-admin-status-report-desktop.png` | Admin weekly status report | Admin | Copy report text, inspect planned/in-progress/recently completed sections, open board, open list, sign out | Concise and launch-useful. The report is clearly a generated preview for copy/paste, with edits expected in email before sending. Matthew should review tone and whether the status grouping matches how DigiColony wants to speak to clients. |
