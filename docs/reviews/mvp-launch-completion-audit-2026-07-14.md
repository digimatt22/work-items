...
| --- | --- | --- | --- |
| Initialize folder as a Git repository. | `git status --short --branch` shows `main`, and `git log --oneline --decorate -6` shows local commits on `main`. | Proven complete locally | Remote is still missing, so shared PR workflow is incomplete. |
