...
| Outputs or state transitions | `node_modules/`, `pnpm-lock.yaml`, generated Prisma client, running Next.js dev server, and inert pending Digi-Portal bindings when the admin-only diagnostic flag is enabled                                                         |
...
| Unknowns / follow-up         | CI provider and durable asset storage remain undecided; use the repository-pinned package-manager version when normalizing local dependencies                                                                                       |
...
| Secrets                      | `~/.config/sheldon/secrets/digicolony-client-ops.env` on Sheldon, mode `0600`; required names are `DATABASE_URL`, `AUTH_SECRET`, `AUTH_TRUST_HOST`, and `AUTH_URL`; values must never enter the repo or deployment archive                                                |
...
| Human verification           | Review the live portal visually and confirm the intended durable asset-storage strategy before relying on uploads                                                                                                                                                         |
| Owner or reviewer            | DigiColony engineering                                                                                                                                                                                                                                                    |
| Unknowns / follow-up         | Persistent upload storage; release archive exclusions and collision-checked rootless subnet allocation are handled by the Sheldon plugin                                                                                                                                  |
