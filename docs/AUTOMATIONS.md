...
| Inputs                       | `.env` copied from `config/local-development.example`; package manifests; Prisma schema at `packages/db/prisma/schema.prisma`; Digi-Portal URL and independently gated binding/read/mutation flags                                  |
...
| Automated verification       | `pnpm lint`; `pnpm test`; `pnpm build`; local container build when Docker is available; schema-2 package audit, plan, inventory, status, preflight, and dry-run; post-deploy origin/public liveness and dependency readiness probes                                       |
...
## Work Items dependency inventory and readiness

| Field                        | Details                                                                                                                                                                                                                                                                                                                                         |
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Name                         | Work Items PostgreSQL/Garage inventory and readiness                                                                                                                                                                                                                                                                                            |
| Purpose                      | Produce repeatable redacted dependency evidence and distinguish application liveness from stateful dependency usability                                                                                                                                                                                                                         |
| Trigger type                 | Read-only manual inventory and HTTP readiness                                                                                                                                                                                                                                                                                                   |
| Schedule or invocation       | `scripts/inventory-work-items-database-on-sheldon-remote.sh`; `scripts/inventory-work-items-garage-on-sheldon-remote.sh`; `GET /api/health`; `GET /api/ready`; `pnpm --filter @digicolony/db storage:verify-isolation` with a real foreign bucket; `scripts/test-sheldon-readiness-containers.sh`; `scripts/test-server-action-release-skew.sh` |
| Inputs                       | Server-side `DATABASE_URL` and `S3_*` values; `GARAGE_DENIED_BUCKET` names a distinct existing application bucket only for the isolation check                                                                                                                                                                                                  |
| Secrets                      | Read from the mode-`0600` Sheldon environment file; never printed. The manifest lists required names only                                                                                                                                                                                                                                       |
| Systems touched              | Read-only PostgreSQL queries, Garage admin metadata reads, S3 `HeadBucket`, and at most one-object foreign-bucket listing attempt expected to return HTTP `403`                                                                                                                                                                                 |
| Outputs or state transitions | Version/role/database metadata, protected counts, Garage health/capacity/object counts, key-policy metadata, volume names, sanitized readiness JSON, and bucket-isolation pass/fail. No intended state transition                                                                                                                               |
| Failure mode                 | Bounded timeout, dependency unavailable, own-bucket denial, unexpected foreign-bucket access, ambiguous non-403 foreign-bucket failure, missing server configuration, or protected environment mode mismatch                                                                                                                                    |
| Retry or recovery            | Diagnose the named dependency read-only. Do not rotate credentials, alter policy, restart services, or recreate state as a diagnostic shortcut                                                                                                                                                                                                  |
| Automated verification       | Database/web unit tests; script syntax; local disposable PostgreSQL/Garage failure and recovery; candidate auth/permissions/file flow; explicit foreign-bucket `403`; non-root/resource checks; A-to-B stale-tab recovery; candidate `/api/ready` returns `200` only when both dependencies are usable and `503` otherwise                      |
| Human verification           | Review the live redacted inventory and approve any database backup/restore, Garage fixture/policy, secret, network, container, deployment, or rollback operation separately                                                                                                                                                                     |
| Owner or reviewer            | DigiColony engineering                                                                                                                                                                                                                                                                                                                          |
| Unknowns / follow-up         | Current Garage has no second application bucket for a live `403` proof; latest Work Items PostgreSQL full dump lacks isolated restore evidence; schema-2 commands await released Sheldon Deploy 0.2.0                                                                                                                                           |
...
| Automated verification       | Garage status/stats, bucket/key checks, application readiness, own-bucket/foreign-bucket isolation, provider-aware unit tests, and `tests/e2e/project-deliverable-live.spec.ts` |
